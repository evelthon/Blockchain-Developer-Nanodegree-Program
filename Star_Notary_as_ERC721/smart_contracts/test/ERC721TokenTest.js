...
contract('ERC721Token', accounts => {
    ...
    describe('Can create a token', () => {
        ...
        it('balance of user1 is incremented by 1', async function () {
            let balance = await this.contract.balanceOf(user1)
            assert.equal(balance.toNumber(), 1)
        })

        /*
        Events get stored as logs. We can get the transfer event form looking at the first log 
        of the transaction receipt. We comprare the event name to "Transfer"
        */
        it('emits the correct event during creation of the new token', async function () {
